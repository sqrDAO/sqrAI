import { generateText, IAgentRuntime, parseJSONObjectFromText } from "@ai16z/eliza"; // Assuming generateText is a function from the eliza package
import PostgresSingleton from "./services/pg";

const messageCompletionFooter = "\nResponse format should be formatted in a JSON block like this:\n```json\n{ \"name\": string, \"owner\": string}\n```";

export async function extractRepoNameAndOwner(runtime: IAgentRuntime, text: string): Promise<{ name: string | null, owner: string | null }> {
  const context = `
...
export async function getRepoByNameAndOwner(name: string, owner: string): Promise<any> {
...
export async function queryRelatedCodeFiles(runtime: IAgentRuntime, repoId: string, questionEmbedding: number[]): Promise<any[]> {
  const pgClient = await PostgresSingleton.getInstance().getClient();
  try {
    const result = await pgClient.query(
